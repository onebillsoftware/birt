...
BirtSimpleExportDataDialog.prototype = Object
		.extend(
				new AbstractBaseDialog(),
				{
					...
					initialize : function(id) {
						...
						// Closures
						this.__neh_switchResultSet_closure = this.__neh_switchResultSet
								.bindAsEventListener(this);
						this.__neh_click_exchange_closure = this.__neh_click_exchange
								.bindAsEventListener(this);
						this.__neh_dblclick_src_closure = this.__neh_dblclick_src
								.bindAsEventListener(this);
						this.__neh_dblclick_dest_closure = this.__neh_dblclick_dest
								.bindAsEventListener(this);
						this.__neh_click_src_closure = this.__neh_click_src
								.bindAsEventListener(this);
						this.__neh_click_dest_closure = this.__neh_click_dest
								.bindAsEventListener(this);
					 ...
					 * 
					 * @id, toolbar id (optional since there is only one
					 *      toolbar)
					 * @return, void
					 */
					__installEventHandlers : function(id) {
						var oSelects = this.__instance
								.getElementsByTagName('select');
						Event.observe(oSelects[0], 'change',
								this.__neh_switchResultSet_closure, false);

						// Initialize exchange buttons
						var oInputs = this.__instance
								.getElementsByTagName('input');
						for ( var i = 0; i < oInputs.length; i++) {
							Event.observe(oInputs[i], 'click',
									this.__neh_click_exchange_closure, false);
						}
						...
						if (oSelects.length > 2) {
							Event.observe(oSelects[1], 'dblclick',
									this.__neh_dblclick_src_closure, false);
							Event.observe(oSelects[2], 'dblclick',
									this.__neh_dblclick_dest_closure, false);
							Event.observe(oSelects[1], 'change',
									this.__neh_click_src_closure, false);
							Event.observe(oSelects[2], 'change',
									this.__neh_click_dest_closure, false);
						}
					},
					...
					__neh_click_exchange : function(event) {
						...
						if (oSC.id == 'exportDataEncoding_other') {
							...
							case 'Add': {
								if (oSelects[1].options.length > 0) {
									this.moveSingleItem(oSelects[1],
											oSelects[2]);
								}
								break;
							}
							case 'Remove': {
								if (oSelects[2].options.length > 0) {
									this.moveSingleItem(oSelects[2],
											oSelects[1]);
								}
								break;
							}
					...
					/**
					 * Native event handler for double click source select
					 * element.
					 */
					...
					/**
					 * Native event handler for double click dest select
					 * element.
					 */
					...
					__updateButtons : function() {
						...
						if (!rtl) {
							oInputs[0].src = canAdd ? "birt/images/AddAll.gif"
									: "birt/images/AddAll_disabled.gif";
						} else {
							oInputs[0].src = canAdd ? "birt/images/AddAll_rtl.gif"
									: "birt/images/AddAll_disabled_rtl.gif";
						}
						oInputs[0].style.cursor = canAdd ? "pointer"
								: "default";

						if (!rtl) {
							oInputs[1].src = canAdd && srcSelectedIndex >= 0 ? "birt/images/Add.gif"
									: "birt/images/Add_disabled.gif";
						} else {
							oInputs[1].src = canAdd && srcSelectedIndex >= 0 ? "birt/images/Add_rtl.gif"
									: "birt/images/Add_disabled_rtl.gif";
						}
						oInputs[1].style.cursor = canAdd ? "pointer"
								: "default";

						if (!rtl) {
							oInputs[2].src = canRemove
									&& destSelectedIndex >= 0 ? "birt/images/Remove.gif"
									: "birt/images/Remove_disabled.gif";
						} else {
							oInputs[2].src = canRemove
									&& destSelectedIndex >= 0 ? "birt/images/Remove_rtl.gif"
									: "birt/images/Remove_disabled_rtl.gif";
						}
						oInputs[2].style.cursor = canRemove ? "pointer"
								: "default";

						if (!rtl) {
							oInputs[3].src = canRemove ? "birt/images/RemoveAll.gif"
									: "birt/images/RemoveAll_disabled.gif";
						} else {
							oInputs[3].src = canRemove ? "birt/images/RemoveAll_rtl.gif"
									: "birt/images/RemoveAll_disabled_rtl.gif";
						}
						oInputs[3].style.cursor = canRemove ? "pointer"
								: "default";

						oInputs[4].src = canRemove && destSelectedIndex >= 0 ? "birt/images/Up.gif"
								: "birt/images/Up_disabled.gif";
						oInputs[4].style.cursor = canRemove ? "pointer"
								: "default";

						oInputs[5].src = canRemove && destSelectedIndex >= 0 ? "birt/images/Down.gif"
								: "birt/images/Down_disabled.gif";
						oInputs[5].style.cursor = canRemove ? "pointer"
								: "default";

						if (canExport) {
							this.__setDisabled('exportDataEncodingSetting',
									false);
					...
					moveSingleItem : function(sel_source, sel_dest) {
						...
						for ( var i = 0; i < sel_source.options.length; i++) {
							if (sel_source.options[i].selected) {
								...
								sel_dest.options[sel_dest.options.length] = new Option(
					...
					/**
					 * Binding data to the dialog UI. Data includes zoom scaling
					 * factor.
					 * 
					...
					__bind : function(data) {
						...
						for ( var k = 0; k < resultSets.length; k++) {
							...
							var queryNames = resultSet
									.getElementsByTagName('QueryName');
							/*
							 * To select only the table resultset  
							 */
							if (queryNames[0].textContent == "mytable") {
								index = k;
								oSelects[0].options[oSelects[0].options.length] = new Option(
										queryNames[0].firstChild.data);

								this.availableResultSets[k] = {};

								var columns = resultSet
								...
								for ( var i = 0; i < columns.length; i++) {
									...
									/*
									 * To remove the column bindings from the
									 * resultset if present
									 */
									if (columnName[0].firstChild.data != "Column&#32;Binding") {
					...
					__neh_switchResultSet : function() {
						...
						var columns = this.availableResultSets[index];
					...
					__constructForm : function() {
						...
						url = url.replace(/[\/][a-zA-Z]+[?]/, '/'
								+ Constants.SERVLET_EXTRACT + '?');

						// delete some URL parameters
						url = birtUtility.deleteURLParameter(url,
								Constants.PARAM_BOOKMARK);
						url = birtUtility.deleteURLParameter(url,
								Constants.PARAM_INSTANCE_ID);
						...
						if (this.selectedColumns.length > 0) {
							var hiddenSelectedColumnNumber = document
									.createElement('input');
							...
							for ( var i = 0; i < this.selectedColumns.length; i++) {
								var hiddenSelectedColumns = document
										.createElement('input');
								hiddenSelectedColumns.type = 'hidden';
								hiddenSelectedColumns.name = Constants.PARAM_SELECTEDCOLUMN
										+ i;
						...
						var hiddenAsAttachment = document
								.createElement('input');
						...
						var hiddenLocaleNeutral = document
								.createElement('input');
